...
class Packet(BasePacket, _CanvasDumpExtended, metaclass=Packet_metaclass):
    ...
    def do_dissect(self, s):
        ...
        for f in self.fields_desc:
            try:
                ...
                # Field parsing failed; fallback to treating remaining bytes as
                # undecoded payload. Stop dissecting further fields.
                s = _raw
                break
    ...
    def dissect(self, s):
        ...
        try:
            ...
            # Field parsing error during dissection: fallback to leaving
            # the remaining bytes as a raw payload to avoid propagating
            # parsing exceptions.
            try:
                self.payload = Raw(s)
            except Exception:
                # In case Raw construction fails for any reason, keep payload as-is
                pass
            return
        ...
        for t in self.aliastypes:
            for fval, cls in t.payload_guess:
                try:
                    ...
                    # If a field access fails (e.g. parsing issue), skip this guess
                    pass
        ...
        def loop(todo, done, self=self):
            ...
            if todo:
                ...
                try:
                    ...
                    # Skip elements that cannot be retrieved due to parsing errors
                    return
    ...
    def __eq__(self, other):
        ...
        for f in self.fields_desc:
            ...
            try:
                ...
                # If a field cannot be accessed/parsed, consider packets not equal
                return False
    ...
    def haslayer(self, cls, _subclass=None):
        ...
        for f in self.packetfields:
            try:
                ...
                # Skip fields that cannot be retrieved due to parsing errors
                continue
        ...
        **flt,  # type: Any
        ...
        for f in self.packetfields:
            try:
                ...
                # Skip fields that cannot be accessed due to parsing errors
                continue
            if fvalue_gen is None:
                continue
            if not f.islist:
    ...
    def _show_or_dump(
        ...
        while fields:
            ...
            try:
                ...
                # In case of parsing error, represent the field as None
                fvalue = None
        ...
        if self.payload:
            try:
                ...
                # If payload display fails due to parsing errors, skip it.
                pass
    ...
    def decode_payload_as(self, cls):
        ...
        try:
            ...
            # If decoding fails, fallback to a raw payload to avoid propagating
            # parsing exceptions.
            try:
                self.payload = Raw(s)
            except Exception:
                # If Raw construction fails, keep existing payload
                return
            return
        pp = self
    ...
    def command(self):
        ...
        try:
            ...
            # If payload command generation fails (e.g. due to parsing errors),
            # ignore and return the command for the current packet only.
            pc = None
        if pc:
            c += "/" + pc
        return c
