...
class FlagValue(object):
    ...
    def _fixvalue(self, value):
        ...
        if isinstance(value, str):
            # For multi-flag strings split on '+'; for single flags keep the whole
            # string as a single token instead of splitting into characters.
            value = value.split("+") if self.multi else [value]
        if isinstance(value, list):
            y = 0
            for i in value:
                # Guard membership before using index() to avoid ValueError for
                # unknown flag names and provide a clear error message.
                if i in self.names:
